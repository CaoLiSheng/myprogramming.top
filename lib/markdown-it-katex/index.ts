...
export default function katexPlugin ( md: MarkdownIt, options: { throwOnError?: boolean, errorColor?: string, displayMode?: boolean } = {} ): void {
    ...
    const inlineRenderer = function inline_render ( tokens: Token[], idx: number ): string {
        return katexInline ( tokens[idx].content );
    };

    const katexBlock = function katex_block ( latex: string ): string {
        options.displayMode = true;
        try {
            return `<p>${ renderToString ( latex, options )  }</p>`;
        }
        catch ( error ){
            if ( options.throwOnError ){ console.log ( error ); }
            return latex;
    ...
    const blockRenderer = function block_renderer ( tokens: Token[], idx: number ): string {
